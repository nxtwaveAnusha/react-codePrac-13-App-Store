...
class AppStore extends Component {
  ...
  getFilteredAppItems = () => {
    ...
    const searchResult = filteredApps.filter(eachApp =>
  ...
  render() {
    const filteredApps = this.getFilteredAppItems()
    const {activeTabId, searchInput} = this.state
    ...
    return (
      <div className="app-container">
        ...
        <ul className="app-item-container">
          {filteredApps.map(appItemDetails => (
            <AppItem
              appItemDetails={appItemDetails}
              key={appItemDetails.appId}
            />
          ))}
        </ul>
      </div>
